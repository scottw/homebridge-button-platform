...
'use strict';
const { AccessoryDelegate } = require('homebridge-lib/AccessoryDelegate');
const ButtonService = require('./ButtonService');
const BatteryService = require('./BatteryService');

class ButtonAccessory extends AccessoryDelegate {
  constructor(platform, context) {
    const name = String(context.button || 'Button');
    const id = name.toLowerCase().replace(/[^a-z0-9]/giu, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
    const params = {
      name,
      id: id || 'button',
