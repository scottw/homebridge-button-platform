  ...
  async init() {
	...
	function (err, req, res, _next) {
        res.status(500).send('Server error.');
            this.fatal(err && err.stack ? err.stack : String(err));
      }.bind(this)
    );
    ...
    this.app.all(
      ...
      (req, res) => {
        ...
        if (!errors.isEmpty()) {
          ...
          const raw = (req.query?.event ?? req.body?.event);
          const event = typeof raw === 'string' ? raw : undefined;
          ...
          if (req.headers['button-battery-level']) {
            const level = Number(req.headers['button-battery-level']);
            accessory.batteryService.updateBattery(level);
          }
