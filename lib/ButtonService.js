...
'use strict';
const { ServiceDelegate } = require('homebridge-lib/ServiceDelegate');

class ButtonService extends ServiceDelegate {
...
class StatelessProgrammableSwitch extends ButtonService {
  ...
  triggerEvent(event) {
    const C = this.Characteristics.hap.ProgrammableSwitchEvent;
    if ([C.SINGLE_PRESS, C.DOUBLE_PRESS, C.LONG_PRESS].includes(event)) {
